...
## Voice-First Interaction (future direction)

Plan to drop the chat/text interface in favor of fully voice-controlled interaction — speak the question, see the result on screen. No persistent chat history or transcript of what was asked/said needs to be displayed or stored; the user just remembers what they asked (or asks again) rather than scrolling back through a conversation log. Implication: the UI should be built around "ask → see result" as a transient interaction, not a growing message thread.
