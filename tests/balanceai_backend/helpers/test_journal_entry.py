...
class TestHandleCreateOrUpdateJournalEntriesForReceipt:
    ...
    def test_no_entries_from_ocr_leaves_journal_unchanged(self, empty_journal, receipt_path):
        with patch(
            ...
            with patch(
                "balanceai_backend.utils.ocr_util.OcrUtil.executeWithAnthropic",
                return_value=GeneratedJournalEntrySet(entries=[]),
            ):
                with patch("balanceai_backend.helpers.journal_entry_helper.db_update_journal"):
                    result = handle_sync_journal_entries_from_receipt("journal-1", receipt_path)
...
class TestHandleCreateOrUpdateJournalEntriesForTransactions:
    ...
    def test_creates_new_entry_for_added_transaction(self, empty_journal, transactions, entry_data):
        ...
        with patch(
            ...
            with patch(
                "balanceai_backend.helpers.journal_entry_helper.extract_journal_entries_from_transactions",
                ...
                with patch(
                    ...
                    with patch("balanceai_backend.helpers.journal_entry_helper.db_update_journal"):
                        result = handle_sync_journal_entries_from_transactions(
                            "journal-1", transactions
                        )
...
class TestHandleSyncJournalEntriesFromBankStatement:
    ...
    def test_raises_when_journal_not_found(self):
        with patch("balanceai_backend.helpers.journal_entry_helper.find_journals", return_value=[]):
            with pytest.raises(ValueError, match="journal-999"):
                handle_sync_journal_entries_from_bank_statement(
                    "journal-999", "/path/to/statement.pdf"
                )
    ...
    def test_creates_new_entries_for_single_transaction(
        ...
        with patch(
            ...
            with patch(
                "balanceai_backend.helpers.journal_entry_helper.get_parser",
                return_value=mock_parser,
            ):
                with patch(
                    "balanceai_backend.helpers.journal_entry_helper.extract_journal_entries_from_bank_statement_transaction",
                    return_value=[expense_entry_data, cash_entry_data],
                ):
                    with patch(
                        "balanceai_backend.helpers.journal_entry_helper.finder_find_journal_entry",
                        return_value=None,
    ...
    def test_updates_existing_entry_preserving_id(
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "balanceai_backend.helpers.journal_entry_helper.extract_journal_entries_from_bank_statement_transaction",
                    return_value=[expense_entry_data],
                ):
                    with patch(
                        "balanceai_backend.helpers.journal_entry_helper.finder_find_journal_entry",
                        return_value=bank_statement_existing_entry,
                    ):
                        with patch(
                            "balanceai_backend.helpers.journal_entry_helper.db_update_journal"
                        ):
    ...
    def test_multiple_entries_per_transaction(
        self, empty_journal, sample_transaction, expense_entry_data, cash_entry_data
    ):
        mock_parser = self._make_parser([sample_transaction])
    ...
    def test_multiple_transactions_all_entries_accumulated(
        self, empty_journal, expense_entry_data, cash_entry_data
    ):
        txn1 = Transaction(
            id="txn-1",
            account_id="acct-1",
        ...
        txn2 = Transaction(
            id="txn-2",
            account_id="acct-1",
            posting_date=datetime.date(2026, 1, 26),
            description="Shell Gas",
            amount=Decimal("-21.77"),
            previous_balance=Decimal("988.00"),
            new_balance=Decimal("966.23"),
        )
        ...
        def extract_side_effect(txn):
            return (
                [expense_entry_data, cash_entry_data]
                if txn.id == "txn-1"
                else [gas_expense, gas_cash]
            )
    ...
    def test_storage_update_called_once(
        self, empty_journal, sample_transaction, expense_entry_data
    ):
        mock_parser = self._make_parser([sample_transaction])
    ...
    def test_recategorizes_null_category_entries(self, empty_journal, sample_transaction):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "balanceai_backend.helpers.journal_entry_helper.extract_journal_entries_from_bank_statement_transaction",
                    return_value=[null_entry],
                ):
                    with patch(
                        "balanceai_backend.helpers.journal_entry_helper.generate_transaction_category",
                        return_value=null_entry,
                    ) as mock_recategorize:
                        with patch(
                            "balanceai_backend.helpers.journal_entry_helper.finder_find_journal_entry",
                            return_value=None,
                        ):
    ...
    def test_skips_recategorization_when_all_entries_categorized(
        self, empty_journal, sample_transaction, expense_entry_data, cash_entry_data
    ):
        mock_parser = self._make_parser([sample_transaction])
    ...
    def test_recategorized_entry_is_used_in_journal(self, empty_journal, sample_transaction):
        ...
        recategorized = null_entry.model_copy(
            update={
    ...
    def test_only_null_category_entries_are_recategorized(
        self, empty_journal, sample_transaction, expense_entry_data
    ):
        null_entry = GeneratedJournalEntry(
            date=datetime.date(2026, 1, 25),
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "balanceai_backend.helpers.journal_entry_helper.extract_journal_entries_from_bank_statement_transaction",
                    return_value=[expense_entry_data, null_entry],
                ):
                    with patch(
                        "balanceai_backend.helpers.journal_entry_helper.generate_transaction_category",
                        return_value=null_entry,
    ...
    def test_all_null_category_entries_are_recategorized(self, empty_journal, sample_transaction):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "balanceai_backend.helpers.journal_entry_helper.extract_journal_entries_from_bank_statement_transaction",
                    return_value=[null_entry_1, null_entry_2],
                ):
                    with patch(
                        "balanceai_backend.helpers.journal_entry_helper.generate_transaction_category",
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        "balanceai_backend.helpers.journal_entry_helper.generate_transaction_category",
                        side_effect=[FakeRateLimitError(), recategorized],
                    ) as mock_recategorize:
                        with patch(
                            "balanceai_backend.helpers.journal_entry_helper.finder_find_journal_entry",
                            return_value=None,
                            ...
                            with patch(
                                ...
                                with patch(
                                    "balanceai_backend.helpers.journal_entry_helper.anthropic"
                                ) as mock_anthropic:
                                    mock_anthropic.RateLimitError = FakeRateLimitError
                                    with patch(
                                        "balanceai_backend.helpers.journal_entry_helper.time.sleep"
                                    ):
                                        handle_sync_journal_entries_from_bank_statement(
                                            "journal-1", "/path/to/statement.pdf"
                                        )
    ...
    def test_null_category_entry_still_added_when_recategorization_returns_unchanged(
        self, empty_journal, sample_transaction
    ):
        null_entry = GeneratedJournalEntry(
            date=datetime.date(2026, 1, 25),
            account=JournalAccount.NON_ESSENTIALS_EXPENSE,
    ...
    def _sync_with_n_entries(self, empty_journal, n):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "balanceai_backend.helpers.journal_entry_helper.extract_journal_entries_from_bank_statement_transaction",
                    side_effect=lambda txn: [entries[transactions.index(txn)]],
                ):
                    with patch(
                        "balanceai_backend.helpers.journal_entry_helper.finder_find_journal_entry",
                        return_value=None,
