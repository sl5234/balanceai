...
from balanceai_backend.models.journal import GeneratedJournalEntry, GeneratedJournalEntrySet, JournalAccount
...
class TestGenerateTransactionCategory:
    def test_returns_entry_with_category_from_cache(self, uncategorized_entry):
        ...
        with patch("balanceai_backend.utils.journal_entry_util.load_merchant_context_cache", return_value=cache):
            with patch("balanceai_backend.utils.journal_entry_util.tavily_service.search") as mock_tavily:
                with patch("balanceai_backend.utils.journal_entry_util.anthropic_service.messages") as mock_llm:
    ...
    def test_calls_tavily_when_recipient_not_in_cache(self, uncategorized_entry):
        with patch("balanceai_backend.utils.journal_entry_util.load_merchant_context_cache", return_value={}):
            with patch("balanceai_backend.utils.journal_entry_util.tavily_service.search", return_value=None) as mock_tavily:
    ...
    def test_returns_entry_unchanged_and_logs_warning_when_tavily_returns_nothing(self, uncategorized_entry, caplog):
        import logging
        with patch("balanceai_backend.utils.journal_entry_util.load_merchant_context_cache", return_value={}):
            with patch("balanceai_backend.utils.journal_entry_util.tavily_service.search", return_value=None):
                with caplog.at_level(logging.WARNING, logger="balanceai_backend.utils.journal_entry_util"):
                    result = generate_transaction_category(uncategorized_entry)
    ...
    def test_calls_llm_with_tavily_context(self, uncategorized_entry):
        ...
        with patch("balanceai_backend.utils.journal_entry_util.load_merchant_context_cache", return_value={}):
            with patch("balanceai_backend.utils.journal_entry_util.tavily_service.search", return_value="Rudy's is a barbershop."):
                with patch("balanceai_backend.utils.journal_entry_util.anthropic_service.messages", return_value="{}") as mock_llm:
                    with patch("balanceai_backend.utils.journal_entry_util._extract_json", return_value=categorized.model_dump_json()):
                        with patch("balanceai_backend.utils.journal_entry_util.save_merchant_context_cache"):
                            generate_transaction_category(uncategorized_entry)
    ...
    def test_returns_llm_result_with_category(self, uncategorized_entry):
        ...
        with patch("balanceai_backend.utils.journal_entry_util.load_merchant_context_cache", return_value={}):
            with patch("balanceai_backend.utils.journal_entry_util.tavily_service.search", return_value="Rudy's is a barbershop."):
                with patch("balanceai_backend.utils.journal_entry_util.anthropic_service.messages", return_value="{}"):
                    with patch("balanceai_backend.utils.journal_entry_util._extract_json", return_value=categorized.model_dump_json()):
                        with patch("balanceai_backend.utils.journal_entry_util.save_merchant_context_cache"):
                            result = generate_transaction_category(uncategorized_entry)
    ...
    def test_writes_recipient_to_category_cache_when_llm_assigns_category(self, uncategorized_entry):
        ...
        with patch("balanceai_backend.utils.journal_entry_util.load_merchant_context_cache", return_value={}):
            with patch("balanceai_backend.utils.journal_entry_util.tavily_service.search", return_value="Rudy's is a barbershop."):
                with patch("balanceai_backend.utils.journal_entry_util.anthropic_service.messages", return_value="{}"):
                    with patch("balanceai_backend.utils.journal_entry_util._extract_json", return_value=categorized.model_dump_json()):
                        with patch("balanceai_backend.utils.journal_entry_util.save_merchant_context_cache") as mock_save:
                            generate_transaction_category(uncategorized_entry)
    ...
    def test_does_not_write_cache_when_llm_returns_null_category(self, uncategorized_entry):
        with patch("balanceai_backend.utils.journal_entry_util.load_merchant_context_cache", return_value={}):
            with patch("balanceai_backend.utils.journal_entry_util.tavily_service.search", return_value="Rudy's is a barbershop."):
                with patch("balanceai_backend.utils.journal_entry_util.anthropic_service.messages", return_value="{}"):
                    with patch("balanceai_backend.utils.journal_entry_util._extract_json", return_value=uncategorized_entry.model_dump_json()):
                        with patch("balanceai_backend.utils.journal_entry_util.save_merchant_context_cache") as mock_save:
                            generate_transaction_category(uncategorized_entry)
...
class TestExtractJournalEntriesFromBankStatementTransaction:
    def test_calls_llm_with_transaction_json(self, sample_transaction, sample_entry_set):
        ...
        with patch("balanceai_backend.utils.journal_entry_util.load_merchant_context_cache", return_value={}):
            with patch("balanceai_backend.utils.journal_entry_util.anthropic_service.messages", return_value="{}") as mock_llm:
                with patch("balanceai_backend.utils.journal_entry_util._extract_json", return_value=llm_response):
